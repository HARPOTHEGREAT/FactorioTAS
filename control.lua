...
local function pause_toggle() --pause game if unpaused, unpause game if paused
    if not game.tick_paused then
        ...
        player.gui.screen.tas_frame.content_frame.controls_flow.tas_pause_toggle.caption = (game.tick_paused) and {"tas.unpause"} or {"tas.pause"} --flip button label between pause and unpause
    end
